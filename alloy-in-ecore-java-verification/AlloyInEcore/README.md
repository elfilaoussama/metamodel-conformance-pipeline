...
Deep embedding of Alloy-style relational logic into the EMF / Ecore ecosystem.

## Repository layout

- `AlloyInEcore/Source/eu.modelwriter.core.alloyinecore/` — the AlloyInEcore core implementation (parser, translator, Kodkod integration).
- `VerificationEnvironment/` — a standalone CLI that verifies instance models against `.recore` invariants and can emit a JSON report.

## Verification pipeline (recommended entrypoint)

If your goal is to **validate instances against `.recore` invariants**, use the verifier in `VerificationEnvironment/`.

- Windows (PowerShell, native): run `VerificationEnvironment/setup.ps1` then `VerificationEnvironment/run.ps1`.
- Linux/macOS: run `VerificationEnvironment/setup.sh` then `VerificationEnvironment/run.sh`.

See `VerificationEnvironment/README.md` for exact commands and flags (`--details`, `--report`).

### Note on “broken rules” reporting

When a proof-capable native solver is available, the verifier extracts a real Kodkod UNSAT core.
On platforms without prover binaries, the verifier falls back to a proof-less approximation (drop-one-rule checks) so `--details` / `--report` still produce useful per-rule output.

## Recent fixes (instance checking)

These were added to make verification runs robust across real-world instances:

- Primitive literal atoms (string/int/bool/real) are consistently added to the universe/bounds.
- Enum literal atom naming is aligned between metamodel and instance values.
- `{id}` qualifier uniqueness constraints are labeled so violations can be aggregated (e.g., `id_unique(Type.attr)`).

## Credits / License

- AlloyInEcore is released under the MIT License.
    - Copyright (c) 2016, Ferhat Erata <ferhat@computer.org>
    - See `AlloyInEcore/LICENSE`.
- This repository also includes the Kodkod relational solver, which is released under the MIT License.
    - Copyright (c) 2005–present, Emina Torlak
    - See `AlloyInEcore/Source/kodkod/LICENSE`.
- Kodkod can use third-party SAT solvers (e.g., SAT4J, MiniSat, Glucose, (P)Lingeling). Some of these have licenses stricter than MIT; please review their licenses if you redistribute binaries.
