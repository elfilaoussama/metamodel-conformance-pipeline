...
public class AlloyInEcoreParser extends Parser {
	...
	public void saveResource(String filename, String path){
	    ...
	    if (model.getOwnedPackage() != null) {
	        ...
	        // NOTE: On Windows, raw paths like "C:\\...\\file.ecore" are NOT valid EMF URIs.
	        // Use a file URI to ensure ResourceSet can select the correct factory.
	        URI targetUri;
	        try {
	            java.io.File targetFile = new java.io.File(path + filename + ".ecore");
	            java.io.File parent = targetFile.getParentFile();
	            if (parent != null) {
	                parent.mkdirs();
	            }
	            targetUri = URI.createFileURI(targetFile.getAbsolutePath());
	        } catch (Exception ex) {
	            // Best-effort fallback for unusual inputs.
	            targetUri = URI.createURI((path + filename + ".ecore").replace('\\', '/'));
	        }

	        Resource metaResource = metaResourceSet.createResource(targetUri);
	        if (metaResource == null) {
	            throw new IllegalStateException("Failed to create Ecore resource for URI: " + targetUri);
	        }
